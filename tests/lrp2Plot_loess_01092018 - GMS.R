lrp2Plot_loess_gabi = function (model) 
{
  ...
  mean.traj = function(model, fix.eff) {
    times = seq(range(model$data$time, na.rm = T)[1], range(model$data$time, 
                                                            na.rm = T)[2], length.out = 1000)
    if (model$method == "lme") {
      ...
      envir.df = data.frame(matrix(nrow = length(times), 
                                   ncol = length(lhs.vars(model$fixedFormula)) + 
                                     1))
  ...
  conf.band = function(model, rand.eff, res.var) {
    ...
    if (model$method == "lme") {
      ...
      for (i in 1:length(times)) {
        envir.for.model = data.frame(time = times[i], 
                                     rand.eff)
        names(envir.for.model)[names(envir.for.model) == 
                                 "time"] = time.metric
  ...
  model$data$groupingName = dat[[attr(terms(splitFormula(model$group, 
                                                         "~")[[1]]), "term.labels")]]
  ...
  p = ggplot(data = curve.df, aes(x = time, y = y, group = grp, 
                                  color = "black", linetype = ltype)) + geom_smooth(se = F, method = "loess") + ####Gabi: method = "loess" (Sarfaraz rec)
    theme_bw() + guides(linetype = F) + facet_wrap(~node, 
                                                   scales = "free") + labs(title = paste("Node"), x = time.var, 
                                                                           y = out.name) + xlim(min(curve.df$time), max(curve.df$time)) + 
    ylim(min(c(curve.df$y, model$data$y)), max(c(curve.df$y, 
                                                 model$data$y))) + scale_color_manual(labels = sort(unique(model$leaf_node)), 
                                                                                      values = c("black")) + theme(axis.line = element_line(), 
                                                                                                                   ...
                                                                                                                   legend.position = "none") + geom_line(data = model$data, 
                                                                                                                                                         aes(x = time, y = responseName, group = groupingName, 
